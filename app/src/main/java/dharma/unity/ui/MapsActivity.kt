...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    lateinit var contactsNumberList: MutableList<ContactsData>
...
//    var freindB = MarkerOptions().position(LatLng(21.2514, 81.6296)).title("Freind B")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        contactsNumberList = mutableListOf()
    ...
    private fun loadListMarkersData() {
        ...
        for(obj in alertNeedHelpList)
        {
            ...
            if(contactsNumberList.any{x-> x.phoneNo == obj.phoneNo } && userPhoneNo != obj.phoneNo) {
                var friendMarker = MarkerOptions().position(LatLng(lat, lang)).title(name)
                friendList.add(friendMarker)
            }
            else{
...
//                Toast.makeText(this@MapsActivity, obj.phoneNo, Toast.LENGTH_LONG).show()
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if (item.itemId == R.id.action_show_friends) {
            ...
            for (obj in friendList)
                mMap!!.addMarker(obj).setIcon(
                    BitmapDescriptorFactory.fromBitmap(
                        getMarkerBitmapFromView(R.drawable.ic_friend)
                    )
                )
        }
...
//                        Toast.makeText(this@MapsActivity, "Connected getting Contacts", Toast.LENGTH_LONG).show()

                        if (contact != null && contact.phoneNo!=userPhoneNo) {
//                            Toast.makeText(this@MapsActivity, "adding"+ contact.name, Toast.LENGTH_LONG).show()
                            contactsNumberList.add(contact)
                        }
                    }

//                    Toast.makeText(this@MapsActivity, "All Contacts added", Toast.LENGTH_LONG).show()

                } else
                    Toast.makeText(this@MapsActivity, "No Personal Contacts", Toast.LENGTH_LONG).show()
            }
        })
    }
            ...
            override fun onDataChange(p0: DataSnapshot) {
                if (p0!!.exists()) {
                    ...
                    for (h in p0.children) {
                        ...
                        if (alert != null && alert.phoneNo!=userPhoneNo) {
                            alertNeedHelpList.add(alert)
                            var mymsg = alert.msg
                        }
                    }
                } else
                    Toast.makeText(this@MapsActivity, "No Emergency", Toast.LENGTH_LONG).show()
            }
        })
